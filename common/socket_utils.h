...
namespace Common
{
    ...
    [[nodiscard]] inline auto createSocket(Logger &logger, const SocketCfg &socket_cfg) -> int
    {
        ...
        {
            // getaddrinfo() may return multiple candidate addresses; this loop applies the
            // same socket configuration steps to each candidate before returning the last fd.
            ...
            if (socket_cfg.is_listening_)
            {
                // Listening UDP sockets bind to INADDR_ANY here, while TCP uses the
                // address chosen by getaddrinfo().
