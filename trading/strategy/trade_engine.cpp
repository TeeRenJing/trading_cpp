...
namespace Trading
{
    ...
    auto TradeEngine::sendClientRequest(const Exchange::MEClientRequest *client_request) noexcept -> void
    {
        ...
        ASSERT(outgoing_ogw_requests_->tryPush(*client_request),
               "TradeEngine: outgoing client request queue is full");
    }
    ...
    auto TradeEngine::run() noexcept -> void
    {
        ...
        while (run_)
        {
            for (auto client_response = incoming_ogw_responses_->front(); incoming_md_updates_->size() > 0 && client_response; client_response = incoming_ogw_responses_->front())
            {
            ...
            for (auto market_update = incoming_md_updates_->front(); incoming_md_updates_->size() > 0 && market_update; market_update = incoming_md_updates_->front())
            {
                logger_.log("%:% %() % Processing %\n", __FILE__, __LINE__, __FUNCTION__, Common::getCurrentTimeStr(&time_str_),
