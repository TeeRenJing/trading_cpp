...
namespace Exchange
{
    ...
    auto SnapshotSynthesizer::addToSnapshot(const MDPMarketUpdate *market_update)
    {
        ...
        // The snapshot cache is laid out as [ticker][order_id] so the latest
        // visible state can be serialized without walking matching-engine books.
        ...
        // Snapshot correctness depends on processing the same ordered stream
        // that subscribers see on the incremental feed.
    ...
    auto SnapshotSynthesizer::publishSnapshot()
    {
        ...
        // The snapshot protocol brackets a full-image transfer with START/END
        // markers so a consumer knows when it has a complete point-in-time view.
        ...
        for (size_t ticker_id = 0; ticker_id < ticker_orders_.size(); ++ticker_id)
        {
            ...
            // CLEAR tells the receiver to discard any stale orders for this ticker
            // before replaying the fresh image below.
    ...
    void SnapshotSynthesizer::run()
    {
        ...
        while (run_)
        {
            // This is the consumer half of an SPSC queue. `front()` lets us read
            // the current element without copying; `pop()` retires it after use.
            for (auto market_update = snapshot_md_updates_->front(); snapshot_md_updates_->size() && market_update; market_update = snapshot_md_updates_->front())
            {
