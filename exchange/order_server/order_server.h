    ...
    public:
        ...
        auto run() noexcept
        {
            ...
            while (run_)
            {
                ...
                // Matching-engine responses are already sequenced per client by
                // the time they reach this queue. Here we only stamp the next
                // outbound sequence number and push bytes to the TCP socket.
                for (auto client_response = outgoing_responses_->front(); outgoing_responses_->size() && client_response; client_response = outgoing_responses_->front())
                {
        ...
        auto recvCallback(TCPSocket *socket, Nanos rx_time) noexcept
        {
            ...
            {
                size_t i = 0;
                // The TCP buffer may contain several fixed-width requests, so we
                // peel off whole messages and leave any tail bytes for next poll.
                for (; i + sizeof(OMClientRequest) <= socket->next_rcv_valid_index_; i += sizeof(OMClientRequest))
                {
                    ...
                    // Sequence checks catch dropped, duplicated, or reordered
                    // client messages before they ever reach the matching engine.
