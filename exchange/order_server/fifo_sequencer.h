...
namespace Exchange
{
    ...
    public:
        ...
        auto sequenceAndPublish()
        {
            ...
            // Requests can arrive from different sockets in the same poll cycle.
            // We sort that batch by receive timestamp before handing it to the
            // matching engine so the engine sees a single deterministic order.
            ...
            for (size_t i = 0; i < pending_size_; ++i)
            {
                ...
                // `tryPush()` publishes to the engine-facing SPSC queue. A full
                // queue here means the downstream consumer has fallen behind.
                ASSERT(incoming_requests_->tryPush(client_request.request_),
                       "FIFOSequencer: incoming request queue is full");
            }
