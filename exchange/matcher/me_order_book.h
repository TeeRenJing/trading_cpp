...
namespace Exchange
{
    ...
    private:
        ...
        // Fast lookup from (client id, client order id) to the live exchange
        // order object. This is what makes cancel requests cheap.
        ClientOrderHashMap cid_oid_to_order_;

        // The book stores two linked structures:
        // 1. price levels (`MEOrdersAtPrice`)
        // 2. individual orders inside each price level (`MEOrder`)
        //
        // Pools avoid per-order heap allocation during trading.
    ...
    private:
        ...
        {
            // Client order ids identify the order from the client's perspective.
            // Market order ids identify the accepted order inside the exchange.
            return next_market_order_id_++;
        }

        auto priceToIndex(Price price) const noexcept
        {
            // This is a compact hash into the fixed-size price-level table, not a
            // guarantee that every distinct price gets its own unique slot.
        ...
        auto addOrdersAtPrice(MEOrdersAtPrice *new_orders_at_price) noexcept
        {
            ...
            // Price levels themselves are kept in a circular doubly linked list.
            // Best bid is at the head of the bid list, best ask at the head of the
            // ask list. That makes top-of-book access O(1).
            ...
            else
            {
                ...
                { // add new_orders_at_price before target.
                    ...
                    {
                        // A better-priced level becomes the new head of book.
        ...
        auto getNextPriority(Price price) noexcept
        {
            ...
            // Within a single price level, smaller priority means earlier arrival.
            // This is the "time" part of price-time priority.
        ...
        auto addOrder(MEOrder *order) noexcept
        {
            ...
            if (!orders_at_price)
            {
                // First order at this price creates a brand-new price level.
            ...
            {
                // Resting orders at the same price are also stored in arrival order.
                // Appending at the tail preserves FIFO matching at that price level.
