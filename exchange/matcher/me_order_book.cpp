...
namespace Exchange
{
    ...
    auto MEOrderBook::match(TickerId ticker_id, ClientId client_id, Side side, OrderId client_order_id, OrderId new_market_order_id, MEOrder *itr, Qty *leaves_qty) noexcept
    {
        ...
        // A match consumes whichever side has less remaining quantity.
        const auto fill_qty = std::min(*leaves_qty, order_qty);

        // leaves_qty tracks how much of the incoming order is still unmatched.
        *leaves_qty -= fill_qty;
        // order->qty_ tracks how much of the resting book order is still available.
        order->qty_ -= fill_qty;

        // Two fill messages are generated: one for the aggressive incoming order
        // and one for the resting order that was already on the book.
        ...
        if (!order->qty_)
        {
            // Once the resting order is fully consumed, it disappears from the book.
        ...
        {
            // Partial fill: same order remains on the book, but with smaller size.
    ...
    auto MEOrderBook::checkForMatch(ClientId client_id, OrderId client_order_id, TickerId ticker_id, Side side, Price price, Qty qty, Qty new_market_order_id) noexcept
    {
        ...
        if (side == Side::BUY)
        {
            // A buy can only trade with asks priced at or below the incoming limit.
        ...
        if (side == Side::SELL)
        {
            // A sell can only trade with bids priced at or above the incoming limit.
    ...
    auto MEOrderBook::add(ClientId client_id, OrderId client_order_id, TickerId ticker_id, Side side, Price price, Qty qty) noexcept -> void
    {
        ...
        // The exchange acknowledges the order first, assigning its internal id.
        client_response_ = {ClientResponseType::ACCEPTED, client_id, ticker_id, client_order_id, new_market_order_id, side, price, 0, qty};
        matching_engine_->sendClientResponse(&client_response_);

        // Try to execute immediately against the opposite side of the book.
        ...
        {
            // Any remainder becomes a resting limit order on the book.
    ...
    {
        // Cancels are validated by looking up the live order pointer from the
        // client+order-id index built when the order was accepted.
        ...
        {
            // Successful cancel removes the order from the internal linked lists,
            // then publishes both the public market-data event and the private
            // client confirmation.
    ...
    auto MEOrderBook::toString(bool detailed, bool validity_check) const -> std::string
    {
        ...
        auto printer = [&](std::stringstream &ss, MEOrdersAtPrice *itr, Side side, Price &last_price, bool sanity_check)
        {
            ...
            // Aggregate quantity at this price level so the printout resembles a
            ...
            if (sanity_check)
            {
                // Ask prices should rise as we walk away from the best ask; bid
                // prices should fall as we walk away from the best bid.
